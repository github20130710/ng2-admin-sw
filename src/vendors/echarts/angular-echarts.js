...
var dt_Regex = /^\d{4}-(0\d|1[0-2])-([0-2]\d|3[01])( ([01]\d|2[0-3])\:[0-5]\d\:[0-5]\d)$/;
function getLinkFunction($http, theme, util, type, $timeout) {
    return function (scope, element, attrs) {
        ...
        function getOptions(data, config, type) {
            ...
            var options = {
                    ...
                    polar:util.isRadarChart(type) ? config.radar.polar :null
                };
            if (!config.showXAxis) {
            ...
            if(!util.isRadarChart(type)){
                delete options.polar;
            }
        ...
        var resizeWaiter = false;//默认resize不用等待
        angular.element("[ng-view]").resize(function() {
            if(scope.chartObj&&scope.config.isResize && !resizeWaiter){//chart允许resize，并且不在等待
                resizeWaiter = true;
                $timeout(function(){
                    var wh = element.parent().width();
                    element.children().width(wh);
...
for (var i = 0, n = types.length; i < n; i++) {
    (function (type) {
        app.directive(type + 'Chart', ['$http', 'theme', 'util','$timeout', function ($http, theme, util, $timeout) {
                    return {
                        restrict: 'EA',
                        template: '<div config="config" style="width:{{forceWidth}}" data="data"></div>',
                        scope: {
                            config: '=config',
                            data: '=data',
                            chartObj: '=?chartObj',
                        ...
                        link: getLinkFunction($http, theme, util, type, $timeout)
                    };
                }]);
    }(types[i]));
}
...
angular.module('angular-echarts.util', []).factory('util', function () {
    ...
    function getAxisTicks(data, config, type) {
        ...
        if (data[0]) {
            angular.forEach(data[0].datapoints, function (datapoint) {
                if(dt_Regex.test(datapoint.x)){
                    datapoint.x = datapoint.x.split(" ").join('\n');
                }
    ...
    function getSeries(data, config, type) {
        ...
        angular.forEach(data, function (serie) {
            ...
            if(type == 'bar'){
                conf = angular.extend(conf, config.bar || {});
            }

            if(type == 'line'){
                conf = angular.extend(conf, config.line || {});
            }
            series.push(conf);
        });
        return series;
